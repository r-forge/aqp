...
## An Example
Consider the situation: you have a collection of pedons that have been correlated to a named soil series (or component) and would like to *objectively* compute a range in characteristics ("low-rv-high" values) and horizon depths. As with most collections of pedon data there may be considerable variation in description style, horizon depths, and number of horizons:

![alt text](genhz-sketch.png)

In this contrived example there are several obvious "micro-correlation" decisions that need to be made before e horizons can be grouped for aggregation. For example, what horizonation prototype will be used (e.g. A-Bt1-Bt2-Bt3-Cr-R)? Does it make sense to group {Bt3, Bt4, BCt, CBt} horizons for aggregation? Along those lines, what about grouping {Bt3, 2Bt3} horizons? Do {BA, AB} horizons occur frequently enough to be included in the horizonation prototype?
