...
## Your Data
The following code block demonstrates how to pull data in using the fetchNASIS() function from the soilDB package.  Once the data is loaded into R by fetchNASIS(), the user would substitute their own data for loafercreek dataset used in the example code in this document.  
...
f <- fetchNASIS()
# subset the data by taxon name - enter your taxon name
f <- f[grep(pattern='ENTER_YOUR_TAXON_NAME', f$taxonname, ignore.case=TRUE), ]
# to run the same code in this document on your data, simply replace loafercreek with your data in 'f'
loafercreek <- f
