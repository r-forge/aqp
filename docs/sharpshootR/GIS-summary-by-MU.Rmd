...
# extract polygons for a single map unit, map unit symbol "7089" in this case
 ...
 * "maat": 4 km resolution, mean annual air temperature (1981--2010 PRISM)
 * "ppt": 4 km resolution, mean annual precipitation (1981--2010 PRISM)
 * "wb": 800 meter resolution, mean of annual monthly differences between precipitation and potential ET (PRISM)
 * "elev": 10 meter resolution, USGS DEM
 * "slope": 10 meter resolution, 3x3 classic slope map derived from "elev"
 * "aspect": 10 meter resolution, 3x3 classic aspect map derived from 10 meter "elev"
...
# load into RAM, if the estimates size of all rasters is small than available memory
if(est < memory.size())
...
poly.stats <- ddply(d, c('pID', 'variable'), .fun=summarize, m=round(median(value, na.rm=TRUE)))
