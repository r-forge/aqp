...
You probably need to install some packages before this will work. Have a look at [this tutorial](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/soilDB/SDA-tutorial.html?root=aqp) to get setup.
...
OK, so how to the fuzzy numbers (`interplr` values) for these components compare? Box and whisker plots are the simplest, but sometimes density plots are helpful for viewing the complete distribution. Looks like Auburn and Dunstone are pretty similar, at least in terms of these interpretations.
...
Here is a crazy idea, what if we could sort a collection of soil series based on a subset of relevant interpretations? We can, as long as some assumptions are made:
 ...
 3. in aggregate, the collection of components named for a soil series will approximate the central tendancy of that series
