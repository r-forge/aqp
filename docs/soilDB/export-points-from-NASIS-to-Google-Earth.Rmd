...
Viewing Pedon Data from NASIS in Google Earth
===============================
`r format(Sys.time(), "%Y-%m-%d")`
Jay Skovlin

This document demonstrates how to use the [soilDB](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/soilDB/soilDB-Intro.html?root=aqp) package to export pedon locations from the local NASIS database to generate KML files for viewing in Google Earth.
...
# subsetting to columns from the site data that may useful as labels
s <- s[, c("site_id", "pedon_id", "obs_date", "pedlabsampnum", "taxonname", "hillslope_pos", "elev_field", "slope_field", "aspect_field", "plantassocnm", "bedrckdepth", "bedrock_kind", 'pmkind', 'pmorigin')]

# generate KML file of site locations
# set KML file path
...
![GE_image](./GE_example1.jpg)


### Building spacetime into Pedon data KML files using site observation date information
The following example uses the site observation date/time field to animate a KML file using the timeslider option in Google Earth.  This can be useful if you are interested in viewing the chronologic order in which pedons were sampled.

```{r example2, eval=FALSE}
# set the timespan begin and end option and use paste to concatenate site_id with taxonname fields
...
The following KML uses the observation date/time field to animate the KML file using the timeslider option in Google Earth.  This can be handy if you are interested in viewing the chronologic order in which pedons were sampled.

```{r example3, eval=FALSE}
# build URL to NASIS Web Pedon Description Report
s$report_link <- paste('<a href="https://nasis.sc.egov.usda.gov/NasisReportsWebSite/limsreport.aspx?report_name=Pedon+Description+html+(userpedid)&pedon_id=', s$pedon_id, '">Pedon Description Report</a>', sep='')
