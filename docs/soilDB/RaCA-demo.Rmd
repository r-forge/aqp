...
RaCA data are queried via HTTP request to the SoilWeb server, with query parameters encoded as [URL parameters](http://en.wikipedia.org/wiki/Query_string). By appending parameters to the base URL [`http://casoilresource.lawr.ucdavis.edu/soil_web/rca/rca_query.php`] it is possible to query "site-level" data associated with soils correlated to the Auburn series like this:
...
There are 5 URL parameters that can be used to refine RaCA data queries:
  ...
  * bbox=**-120,37,-122,38** ------*return only those data within the specified bounding box*
...
points(y ~ x, data=site(r.bbox$pedons), col='DarkGreen', cex=0.5, pch=15)
...
### Soil Organic Carbon Concentration / Stock Data
```{r conc-stock, fig.width=8, fig.height=5, eval=TRUE}
...
# visualize
bwplot(taxonname ~ value_100cm, data=site(s))
```
...
# plot aggregate data
xyplot(
  top ~ p.q50, data=a, main='Auburn', ylab='Depth (cm)',
