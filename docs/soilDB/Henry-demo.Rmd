...
# TODO: copy user site ID + depth over to $soiltemp for plotting
...
xyplot(sensor_value ~ date_time | sid, data=x$soiltemp, type=c('l', 'g'), as.table=TRUE, layout=c(1,6), xlab='Date', ylab='Deg C')
```


Another approach for investigating data gaps, blue: data, grey: no data.
```{r plot-time-series-levelplot, fig.width=9, fig.height=9}
levelplot(factor(!is.na(sensor_value)) ~ doy * factor(year) | sid, 
...
levelplot(sensor_value ~ doy * factor(year) | sid, main='Daily Soil Temperature at 50cm\nDeg. C',
...
as.data.frame(x$sensors)[, c('user_site_id', 'name', 'MAST', 'Winter', 'Summer', 'STR', 'functional.yrs', 'complete.yrs')]
```
...
library(rgdal)
writeOGR(x$sensors, dsn='foldername', layer='filename', format='ESRI Shapefile')
```
