...
Spatial queries can be included in SQL statements submitted to SDA as long as the geometry has first been transformed to WGS84 geographic coordinates and formatted as "[well known text](https://en.wikipedia.org/wiki/Well-known_text)" (WKT). The `sp` and `rgdal` packages provide functionality for converting between coordinate systems via `spTransform()`. Coordinate reference system definitions (a "CRS") are typically provided using [proj4](https://en.wikipedia.org/wiki/PROJ.4) notation. You can search for various CRS definitions in a variety of formats using [spatialreference.org/](http://spatialreference.org/). 
...
It is possible to download small collections of SSURGO map unit polygons from SDA using a bounding-box in WGS84 geographic coordinates. SDA will requrn polygons and their map unit keys that **overlap** with the BBOX query.
...
legend('bottomleft', legend=c('query point', '1000m buffer', 'buffer BBOX', 'intersecting polgons', 'overlapping polygons'), col=c('orange', 'red', 'royalblue', 'black', 'grey'), lwd=c(NA, 2, 2, 2, 2), pch=c(15, NA, NA, NA, NA))
box()
```
...
The examples above illustrate how to perform SDA queries using a single spatial filter. Typically we need to perform these queries over a collection of points, lines or polygons. The following examples make use of a special helper function that iterates over a collection of features (usually points) and queries SDA for the intersecting map unit keys and map unit names.

Let's define two custom functions. Copy and paste the entire block of code below into your R script and then run it. 
```{r sda-spatial-query-helper-functions}
# i is a Spatial* object with CRS: WGS84 GCS
make.query <- function(i) {
...
# x is a collection of Spatial* objects
# id is the name of an attribute that contains a unique ID for each feature
query.features <- function(x, id='pedon_id') {
...
The first function `make.query()` will convert a single `Spatial*` (Points, Lines, Polygons) object to WKT and submit a spatial query to SDA, returning intersecting map unit keys. Let's try it using a `SpatialPoints` object.
...
The second function `query.features()` will iterate over a set of `Spatial*` (Points, Lines, Polygons) objects, send a query to SDA, and return the results as a `data.frame` object. This time, our example set of 2 points will also have some fake pedons IDs.
...
query.features(p, id='pedon_id')
```


Let's apply the `query.features()` function to some real data; KSSL pedons correlated to "Auburn". Not all of these pedons have coordinates, so we will have to do some filtering first.
...
mu.data <- query.features(auburn.sp, id='pedlabsampnum')

# join results to original SoilProfileCollection using 'pedlabsamnum'
site(auburn) <- mu.data
```
...
# describe IDs
mtext('user site ID', side=2, line=-1.5)
