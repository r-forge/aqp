...
There are several bedrock kinds that are functionally similar within CA630, so we can group them into a several "generalized bedrock kinds". 
```{r re-group}
# there are many in there that are functionally identical, lets group them
...
For now, lets only aggregate those pedons that we have included within the generalized bedrock classes.
```{r subset}
...
Lattice graphics are used to plot the median property bounded by the 25th and 75th percentiles, with data split into panels according to generalized bedrock type.
```{r make-plots}
...
```{r plot, fig.width=15, fig.height=10, out.width="100%"}
