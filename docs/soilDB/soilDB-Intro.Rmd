...
It can be difficult to locate all of the dependencies required for sending/processing SOAP requests, especially on UNIX-like operating systems. Some of the windows binary packages for the dependencies can be found [here](http://www.stats.ox.ac.uk/pub/RWin/bin/windows/contrib/2.15/). In order to send queries to the SDA system, the [SSOAP](http://www.omegahat.org/SSOAP/) and [XMLSchema](http://www.omegahat.org/XMLSchema/) packages are required. These are source packages and can be installed from the command like with `R CMD INSTALL package.tar.gz`. 
...
The `SDA_query()` function submits a query (ANSI SQL) to the SDA web-service using the `SSOAP` package, parses the result, and returns a `data.frame` (rectangular table) object. Note that the [SSOAP](http://www.omegahat.org/SSOAP/) and [XMLSchema](http://www.omegahat.org/XMLSchema/) packages are not installed by default, and must be manually installed.
```{r sda-example, eval=FALSE}
# setup a query
