...
# setup
opts_chunk$set(message=FALSE, warning=FALSE, background='#F7F7F7', dpi=100, fig.align='center', dev='png', tidy=FALSE, verbose=FALSE)
...
Setting up this connection is simple, and outlined in this [guide](https://r-forge.r-project.org/scm/viewvc.php/*checkout*/docs/soilDB/setup_local_nasis.html?root=aqp). Queries submitted to the national database will load pedons or DMU data into your local database. Functions in `soilDB` can only access data in your local database, and do not depend on the selected set. While it is possible to filter pedon or DMU records within **R**, it is generally best to also filter via queries to the national database. In other words, load up your local database with pedons associated with your office, project, or MLRA-- then load these pedons in **R** for further filtering/processing.
